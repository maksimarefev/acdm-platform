...
task("redeemOrder", "Buy for the 'Trade' round")
    ...
    .setAction(async function (taskArgs, hre) {
        ...
        const tx: any = await acdmPlatform.redeemOrder(taskArgs.orderId, { value: taskArgs.value });
        const txReceipt: any = await tx.wait();

        txReceipt.events
            .filter(event => event.event != undefined)
            .forEach(event => {
                if (event.event === "TradeOrder") {
                    ...
                    console.log("Referral payment issued; to: %s, amount: %s", event.args.referrer, event.args.amount.toString());
                }
            });
